...
class TelloARL(Node):
    ...
    def aruco_pose_callback(self, msg):
        ...
        if aruco_pose.position.z < self.distance - self.offset_z:
            ...
            self._twist.linear.x = self.__set_forward(self.offset_z, self.speed_front, aruco_pose.position.z)
    ...
    def __set_forward(self, __speed: float, __dist: float, __distance: float = 0.0):
        __speed /= 2
        if __dist < 1:
            return __speed / (__dist**2)
        elif __dist < 2:
            return __speed * __dist
        else:
            return __speed * 2
